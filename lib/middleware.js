...
module.exports = {
  ...
  writeHtmlFile(req, res, next) {
    ...
    if (!req.body.html) {
      return next(new Error("Request must contain HTML for conversion"));
    }
    fs.writeFile(`${req.temp.name}/html.html`, req.body.html, (err) => {
      // Should we `next(err)` in this case as well?
  ...
  async createFileList(req, res, next) {
    ...
    await Promise.all(
      req.body.attachments.map(async (attachment, i) => {
        if (typeof attachment !== "string") {
          console.log(
            `Attachment content expected a string got "${typeof attachment}" in payload`
          );
          return;
        }
  ...
  mergeFileList(req, res, next) {
    ...
    if (req.filelist.length === 1) {
      ...
      command = "mv";
      args = [`${req.temp.name}/html.pdf`, `${req.temp.name}/pre-stamp.pdf`];
    } else {
      debug("merge files");
      command = "pdftk";
  ...
  addStamp(req, res, next) {
    debug("addStamp started")
    let stampAction = null;

    if (!req.body.stamp) {
      debug("no stamp required, rename pre-stamp to output")
      stampAction = execa("mv", [
        `${req.temp.name}/pre-stamp.pdf`,
      ...
      stampAction = execa("pdftk", args)
    }

    stampAction
      .then(() => {
        debug("addStamp complete")
        next()
      })
      .catch((err) => catchCmdlineError(err, next));
  },
